...
% quality factor = 75.
cover = load('coverdctrnamed.mat');
stego = load('juniwardctrnamed.mat');
...
% Predictions: -1 stands for cover, +1 for stego
predictions = [test_results_cover.predictions; test_results_stego.predictions];
true_labels = [-ones(size(TST_cover,1),1); ones(size(TST_stego,1),1)];

% Convert true_labels and predictions to categorical arrays
true_labels_cat = categorical(true_labels, [-1, 1], {'Cover', 'Stego'});
predictions_cat = categorical(predictions, [-1, 1], {'Cover', 'Stego'});
...
conf_matrix = confusionmat(true_labels_cat, predictions_cat);

% Display confusion matrix
figure;
heatmap({'Cover', 'Stego'}, {'Cover', 'Stego'}, conf_matrix);
title('Confusion Matrix');
xlabel('Predicted');
ylabel('True');


% Calculate precision and recall during testing
num_cover_predicted_correctly = sum(test_results_cover.predictions == -1);
num_stego_predicted_correctly = sum(test_results_stego.predictions == 1);
num_cover_predicted_incorrectly = false_alarms;
num_stego_predicted_incorrectly = missed_detections;

precision_cover = num_cover_predicted_correctly / (num_cover_predicted_correctly + num_cover_predicted_incorrectly);
recall_stego = num_stego_predicted_correctly / (num_stego_predicted_correctly + num_stego_predicted_incorrectly);

fprintf('Precision (Cover): %.4f\n', precision_cover);
fprintf('Recall (Stego): %.4f\n', recall_stego);

% Calculate accuracy during testing
test_results_cover = ensemble_testing(TST_cover, trained_ensemble);
test_results_stego = ensemble_testing(TST_stego, trained_ensemble);
num_cover_correct = sum(test_results_cover.predictions == -1);
num_stego_correct = sum(test_results_stego.predictions == 1);
total_cover = length(test_results_cover.predictions);
total_stego = length(test_results_stego.predictions);
total_samples = total_cover + total_stego;
accuracy = (num_cover_correct + num_stego_correct) / total_samples;
